...
def genbank_classification(accession_num):
    ...
    for feature in seqrecord.features:
        ...
        if not re.search(r"^gp.*", gp):
            gp = feature.qualifiers.get("locus_tag", ["blank"])[0]
        proteins[gp] = Protein(
            id=gp,
            translation=translation,
            locus_tag=feature.qualifiers.get("locus_tag", ["blank"])[0],
            product=product,
            start=start_pos,
            stop=stop_pos,
    ...
    phage = Phage(
        ...
        taxonomy = ", ".join(seqrecord.annotations.get("taxonomy", ["blank"])),
        host = seqrecord.features[0].qualifiers.get("host", ["blank"]),
