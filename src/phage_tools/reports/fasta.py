...
def matching_proteins_fasta(accession_num, keyword_list):
    matching_proteins = search_proteins(accession_num, keyword_list)
...
def multi_phage_matching_fasta(phage_list, keyword_list):
    ...
    for current_phage in phage_list:
        phage = genbank_classification(current_phage)
        fasta_output = matching_proteins_fasta(phage.accession, keyword_list)
...
write_fasta_to_file(phages.maddie, keywords.tails, filename="maddie_tail_proteins.fasta")
